...
usa = usa %>% mutate(export.mln_usd,
                     export.growth = export.mln_usd - lag(export.mln_usd),
                     export.growth.percentage = export.growth / lag(export.mln_usd) * 100)
deu = deu %>% mutate(export.mln_usd, 
                     export.growth = export.mln_usd - lag(export.mln_usd),
                     export.growth.percentage = export.growth / lag(export.mln_usd) * 100)
grc = grc %>% mutate(export.mln_usd,
...
Distribucije postotnog rasta izgledaju normalnije pa ćemo njih uzeti za analizu. U daljnjem tekstu ćemo za postotni rast govoriti samo rast.
...
boxplot(deu$gdp.mln_usd,
        ...
        log = "y")
```
Kao i kod uvoza i izvoza, po čistom BDP-u SAD značajno prednjači, dok je razlika između Njemačke i Grčke veća od one između SAD-a i Njemačke. No, ovaj prikaz nije mjerodavan što se tiče razvijenosti i standarda života. Na to ukazuje BDP po stanovniku:
>>>>>>> Stashed changes
```{r}
