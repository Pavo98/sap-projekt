...
## Ovisnost ukupnog GDP o neto izvozu

Prvo ćemo uzeti jednostavan model linearne regresije te ispitati ovisnost ukupnog GDP o neto izvozu.

```{r}
usa.model.gdp.net = lm(gdp.pc.usd_cap ~ net.trade, data = usa)
summary(usa.model.gdp.net)
```

```{r}
ggplot(usa, aes(x = net.trade, y = gdp.pc.usd_cap)) +
  geom_jitter() + 
  geom_smooth(method = "lm")

ggplot(fortify(usa.model.gdp.net), aes(x = .hat, y = .resid)) +
  geom_jitter() +
  geom_abline(slope = 0, intercept = 0, col = "steelblue")

qqnorm(usa.model.gdp.net$residuals)
qqline(usa.model.gdp.net$residuals, col = "steelblue", lwd = 2)
```
